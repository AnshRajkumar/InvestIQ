...
```bash
gunicorn investiq_api.wsgi:application \
  --bind 0.0.0.0:8000 \
  --workers 4 \
  --worker-class sync \
  --timeout 120 \
...
Create `/etc/systemd/system/investiq.service`:

```ini
[Unit]
Description=InvestIQ Django Application
...
sudo systemctl enable investiq
sudo systemctl start investiq
```
...
# Run Gunicorn
CMD ["gunicorn", "investiq_api.wsgi:application", "--bind", "0.0.0.0:8000", "--workers", "4"]
```
...
services:
  ...
  web:
    build: ./backend
    command: gunicorn investiq_api.wsgi:application --bind 0.0.0.0:8000 --workers 4
    environment:
      DEBUG: "False"
...
```bash
# PostgreSQL backup
pg_dump -U investiq investiq_db > backup.sql

# Automated daily backup
0 2 * * * pg_dump -U investiq investiq_db | gzip > /backups/db_$(date +\%Y\%m\%d).sql.gz
```
...
```bash
# Restore from backup
psql -U investiq investiq_db < backup.sql
```
...
```bash
# Check status
sudo systemctl status investiq

# Restart
sudo systemctl restart investiq
